<div>
    <form action="{{route('server.store')}}" method="post" id="myModal" class="modal fixed inset-0 z-50 hidden flex items-center justify-center bg-black bg-opacity-65" enctype="multipart/form-data">
        @csrf
        @method('post')
        <div class="bg-[#313338] p-6 rounded max-w-lg w-full relative flex flex-col justify-center" role="document">
            <div class="flex flex-col items-center justify-center">
                <span id="closeModalBtn" class="absolute top-1 right-3 text-gray-400 text-[30px] cursor-pointer hover:text-red-500">&times;</span>
                <h2 class="text-white font-bold mb-2 text-[25px]">Customize Server</h2>
            ...
            <div class="flex flex-col">
                ...
                <input class="bg-zinc-900 text-white rounded-sm my-2 pl-2 py-2 text-[14px]" type="text" name="name" placeholder="eg:UserServer...">
                <p class="text-[10px] text-gray-200">By creating a server,you agree to Discord's Community Guidelines.</p>
            </div>

            <button type="submit" id="saveChangesBtn" class="px-4 py-2 mt-4 text-white bg-slate-500 rounded hover:bg-slate-600">Create</button>
        </div>
    </form>
</div>